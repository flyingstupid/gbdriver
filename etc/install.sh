...
# REBRANDED TO SAPIDLIB NOV-23-07
...
echo "=------------------------------=";
echo "|${BLUE} Welcome to the Sapidlib Install Script${NORMAL} |";
echo "=------------------------------=";
echo;
echo "This script will automatically install and configure";
echo "the Sapidlib mudlib after getting some information from";
echo "you. If you do not wish to continue with this process";
...
echo;
echo "We now need to find out where you'd like to install Sapidlib.";
...
echo "${RED}This script does NOT check for build dependencies.${NORMAL}";
echo "${RED}If the build fails, the mud will not boot (rename files in bin/)";
echo "${RED}**************ADVANCED USER QUESTION****************${NORMAL}";
echo "${CYAN} ONLY ANSWER YES IF YOU KNOW WHAT YOU ARE DOING ${NORMAL}";
echo "${RED}**************ADVANCED USER QUESTION****************${NORMAL}";
...
echo "###############################################################################
#                Sapidlib Mudlib Runtime Configuation File                    #
###############################################################################
...
# (unused currently)
living hash table size : 100" > "$install_dir/etc/config.mud";
...
# BINDIR=/users/somervil/sapidlib/bin
BINDIR=../bin

#  ETCDIR
# Directory with runtime configuration data
# ETCDIR=/users/somervil/sapidlib/etc
ETCDIR=./
...
echo "please visit http://lpmuds.net";

echo;
echo "Bug reports, feature requests, support requests, and patchs can be";
echo "submitted to http://launchpad.net/sapidlib";
echo;
echo "Thank you installing the Sapidlib mudlib.";
