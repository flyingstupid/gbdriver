{
  "__NO_LIGHT__", "",
  "__SANE_EXPLODE_STRING__", "",
  "__SERVER_IP__", "\"192.168.2.2\"",
  "__CUSTOM_CRYPT__", "",
  "__GET_CHAR_IS_BUFFERED__", "",
  "__ARGUMENTS_IN_TRACEBACK__", "",
  "__ARRAY_STATS__", "",
  "__LOCALS_IN_TRACEBACK__", "",
  "__ED_INDENT_SPACES__", "4",
  "__REF_RESERVED_WORD__", "",
  "__SYSMALLOC__", "",
  "__CFG_MAX_CALL_DEPTH__", "150",
  "__PACKAGE_COMPRESS__", "",
  "__SAVE_EXTENSION__", "\".o\"",
  "__LOG_CATCHES__", "",
  "__PROG_REF_TYPE__", "short",
  "__SMALL_STRING_SIZE__", "100",
  "__CALLOUT_CYCLE_SIZE__", "64",
  "__PACKAGE_MATH__", "",
  "__ARRAY_RESERVED_WORD__", "",
  "__PACKAGE_DEVELOP__", "",
  "__SUPPRESS_ARGUMENT_WARNINGS__", "",
  "__LARGEST_PRINTABLE_STRING__", "8192",
  "__CFG_LIVING_HASH_SIZE__", "256",
  "__CACHE_STATS__", "",
  "__CONFIG_FILE_DIR__", "\"../etc\"",
  "__TRAP_CRASHES__", "",
  "__RESTRICTED_ED__", "",
  "__STRING_STATS__", "",
  "__HAS_CONSOLE__", "",
  "__CALLOUT_HANDLES__", "",
  "__CALL_OTHER_WARN__", "",
  "__FLUFFOS__", "",
  "__BINARIES__", "",
  "__CFG_COMPILER_STACK_SIZE__", "600000",
  "__LARGE_STRING_SIZE__", "1000",
  "__HEARTBEAT_INTERVAL__", "1",
  "__PACKAGE_MATRIX__", "",
  "__PRIVS__", "",
  "__COMMAND_BUF_SIZE__", "2000",
  "__OLD_ED__", "",
  "__PACKAGE_PARSER__", "",
  "__THIS_PLAYER_IN_CALL_OUT__", "",
  "__PACKAGE_CONTRIB__", "",
  "__USE_ICONV__", "",
  "__PROFILE_FUNCTIONS__", "",
  "__CFG_MAX_LOCAL_VARIABLES__", "50",
  "__MESSAGE_BUFFER_SIZE__", "4096",
  "__NO_WIZARDS__", "",
  "__CFG_MAX_GLOBAL_VARIABLES__", "256",
  "__USE_32BIT_ADDRESSES__", "",
  "__SAVE_GZ_EXTENSION__", "\".o.gz\"",
  "__PACKAGE_SOCKETS__", "",
  "__NUM_EXTERNAL_CMDS__", "0",
  "__HEART_BEAT_CHUNK__", "64",
  "__INTERACTIVE_CATCH_TELL__", "",
  "__TRACE_CODE__", "",
  "__HAVE_ZLIB__", "",
  "__MAX_SAVE_SVALUE_DEPTH__", "100",
  "__DEFAULT_PRAGMAS__", "PRAGMA_WARNINGS + PRAGMA_STRICT_TYPES + PRAGMA_ERROR_CONTEXT + PRAGMA_SAVE_TYPES",
  "__TRACE__", "",
  "__PACKAGE_ASYNC__", "",
  "__CFG_EVALUATOR_STACK_SIZE__", "3000000",
  "__PACKAGE_MUDLIB_STATS__", "",
  "__APPLY_CACHE_BITS__", "14",
  "__SENSIBLE_MODIFIERS__", "",
  "__MUDLIB_ERROR_HANDLER__", "",
};

#define NUM_OPTION_DEFS 67

